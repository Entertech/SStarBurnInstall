...
if [[ "$EUID" -ne 0 ]]; then
  ...
  if [[ "$REEXEC_ARG_COUNT" -gt 0 ]]; then
    exec sudo -H env PYTHON_BIN="$PYTHON_BIN" SSH_AUTH_SOCK="${SSH_AUTH_SOCK:-}" SSBURN_REPO_URL="$REPO_URL" SSBURN_TRACK_REF="$TRACK_REF" "$SCRIPT_PATH" "${REEXEC_ARGS[@]}"
  fi
  exec sudo -H env PYTHON_BIN="$PYTHON_BIN" SSH_AUTH_SOCK="${SSH_AUTH_SOCK:-}" SSBURN_REPO_URL="$REPO_URL" SSBURN_TRACK_REF="$TRACK_REF" "$SCRIPT_PATH"
fi
